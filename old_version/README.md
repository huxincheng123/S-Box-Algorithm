...
### Build & Run
- Build: `g++ -O2 -std=c++17 -o main main.cpp`
- Input source: by default `main` uses `freopen("three_sbox.txt", "r", stdin);` and logs to `result.txt`. Change the filename in `main.cpp` or comment out `freopen` to read from stdin.
- Example run with a file: `./main < data_703.txt` (or set `freopen` to `data_703.txt`).
...
- 输入来源：默认在 `main` 中 `freopen("three_sbox.txt", "r", stdin);`，并将输出写入 `result.txt`。可修改该文件名或注释掉 `freopen` 以改用标准输入。
- 文件运行示例：`./main < data_703.txt`（或把 `freopen` 路径改成 `data_703.txt`）。
